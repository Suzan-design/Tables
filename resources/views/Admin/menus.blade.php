...
@section('content')
                    ...
                    <table class="table card-table display mb-4 dataTablesCard " id="example5">
                        ...
                        <tbody>
                            @forelse($menus as $menu)
                                <tr>
                                    <td><span class="text-black font-w500">#{{ $menu->id }}</span></td>
                                    <td><span class="btn btn-danger light">{{ $menu->name }}</span></td>
                                    <td><span class="btn btn-danger light">{{ $menu->ar_name }}</span></td>
                                    <td>
                                        <img style="height:16px;width:16px;"src="{{ asset($menu->symbol) }}"
                                            alt="{{ $menu->name }}">
                                    </td>
                                        ...
                                        <div class="dropdown">
                                         ...
                                         document.getElementById('destroy-form-{{ $menu->id }}').submit();">
                                                ...
                                                <a class="dropdown-item edit" href="javascript:void(0);"
                                                    ...
                                                    data-id="{{ $menu->id }}"
                                                    data-name="{{ $menu->name }}"
                                                    data-ar_name="{{ $menu->ar_name }}"
                                                    data-ar_name="{{ $menu->sympol }}"
            ...
            <form method="post" action="{{ route('menus.store') }}" autocomplete="off" enctype="multipart/form-data">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Add a menu</h5>
    ...
    @if ($menus->count() != 0)
        <div class="modal fade" id="edit">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <form id="edit" method="post" action="{{ route('menu_update') }}" autocomplete="off" enctype="multipart/form-data">
                    <input type="hidden" id="edit_id" name="menu_id">
                    {{ method_field('patch') }}
                    {{ csrf_field() }}
                    <div class="modal-content">
                        <div class="modal-header">
        ...
        <div class="modal fade" id="deleteMessageModal">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <form method="post" action="{{ route('menus.destroy', $menu->id) }}" autocomplete="off"
    ...
    <script>
        $(document).ready(function() {
            ...
            // فتح مودال تحرير مستخدم إذا كانت هناك أخطاء في نموذج editUser
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#edit .text-danger').remove(); // إزالة رسائل الخطأ
                    ...
                    var updateUrl = "{{ url('/menus') }}/" + Id; // Update this URL based on your routing
